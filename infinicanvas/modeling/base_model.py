...
class InfiniTensorModel:
    ...
    def make_op(
        ...
        # make op
        self._operators[node_name] = (op_name, attributes)
        # record topo info
    ...
    def load_params(self, data: Dict[str, np.ndarray]):
        if len(self._parameters) != len(data) or set(data.keys()) != set(
            self._parameters.keys()
        ):
            print(
                f"Warning: the number or name of loaded params does not match current model."
            )
